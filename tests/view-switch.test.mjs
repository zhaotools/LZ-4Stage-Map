...
test("sidebar switches between the five stage-map collections", () => {
  ...
  assert.match(pageSource, /switchView\("global"\)\}>全球<\/button>/);
  assert.match(pageSource, /switchView\("crypto7"\)\}>加密<\/button>/);
  assert.match(pageSource, /switchView\("usSelected"\)\}>美股<\/button>/);
  assert.match(pageSource, /switchView\("chinaIndices"\)\}>A股<\/button>/);
  assert.match(pageSource, /switchView\("hkSelected"\)\}>港股<\/button>/);
  assert.ok(pageSource.indexOf(">全球</button>") < pageSource.indexOf(">加密</button>"));
  assert.ok(pageSource.indexOf(">加密</button>") < pageSource.indexOf(">美股</button>"));
  assert.ok(pageSource.indexOf(">美股</button>") < pageSource.indexOf(">A股</button>"));
  assert.ok(pageSource.indexOf(">A股</button>") < pageSource.indexOf(">港股</button>"));
